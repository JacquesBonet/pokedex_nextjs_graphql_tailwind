...
export const fetchPokemons = async (ctx: QueryFunctionContext<['pokemons', QueryPokemonFilter]>) => {
    ...
    return res.pokemon_v2_pokemon.map( elem => (
        { id: elem.id, name: elem.name, types: elem.pokemon_v2_pokemontypes.map(aType => aType.pokemon_v2_type.name)}
    ));
};
